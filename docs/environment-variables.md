...
| `.env.local` | Local dev (Next.js, CLI) | Environment variables for local development. Copy from `.env.example`. Next.js reads this automatically from the project root via `next.config.ts`. Not committed to git. |
